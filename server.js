...
async function load(){
  ...
  loadInProgress=true;
  try{
    const st=await api("/hg/state");if(!st||st.error){const status=document.getElementById("status");if(status)status.textContent="Não foi possível carregar a partida.";return}const g=st.game||{},players=Array.isArray(st.players)?st.players:[],logs=Array.isArray(st.logs)?st.logs:[];applyServerNarrationSettings(st.narration||{});document.body.classList.toggle("hg-running",!admin&&(g.status==="running"||g.status==="ended"));document.getElementById("statusPill").textContent=(statusLabels[g.status]||"AGUARDANDO")+(g.adult_mode?" • +18":"");document.getElementById("phase").textContent=phaseLabel(g.phase||"bloodbath",g.day_number||1);document.getElementById("status").textContent=g.status==="running"?"Partida em andamento":g.status==="ended"?("Vencedor: "+(g.winner||"ninguém")):"Aguardando participantes";const alive=players.filter(p=>Number(p.alive)===1).length;document.getElementById("counts").textContent=players.length+" participantes • "+alive+" vivos • "+Number(st.eventCount||0)+(Number(g.normal_events_enabled??1)===1?" eventos ativos":" eventos de história")+(st.activeScenario?" • História: "+st.activeScenario.name:"")+(st.autoRunning?" • automático ligado":"");renderWinnerPrize(st.winnerPrize||{},g.winner||"",g.status||"");setWinnerPrizeControls(st.winnerPrize||{});normalEventsCurrentlyEnabled=Number(g.normal_events_enabled??1)===1;if(admin){const toggle=document.getElementById("normalEventsToggle"),label=document.getElementById("normalEventsStatus");if(toggle){toggle.textContent=normalEventsCurrentlyEnabled?"Desativar eventos antigos":"Ativar eventos antigos";toggle.className=normalEventsCurrentlyEnabled?"danger":"ok"}if(label){label.textContent=normalEventsCurrentlyEnabled?"⚠️ Eventos normais/antigos: ATIVADOS":"✅ Modo História exclusivo: eventos normais/antigos DESATIVADOS";label.style.color=normalEventsCurrentlyEnabled?"#fca5a5":"#86efac"}}
...
syncEventTimeline(g.id||0,logs,players,g.status)  }finally{
