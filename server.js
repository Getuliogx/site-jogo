...
function setFoldState(id,expand){const body=document.getElementById(id);if(!body)return;const btn=document.querySelector('[data-fold-target=+id+]');body.classList.toggle('collapsed',!expand);if(btn)btn.setAttribute('aria-expanded',expand?'true':'false')}
function toggleFold(id){const body=document.getElementById(id);if(!body)return;setFoldState(id,body.classList.contains('collapsed'))}
function openFold(id){setFoldState(id,true)}
function openWinnerPrizePanel(){const box=document.getElementById("winnerPrizeAdminCard");if(!box)return;openFold('winnerPrizeAdminContent');box.scrollIntoView({behavior:"smooth",block:"start"});box.animate([{outline:"4px solid #c084fc"},{outline:"0 solid transparent"}],{duration:1200,easing:"ease-out"})}
